...
const createFeedback = async (req, res, next) => {
    try {
        const { createdBy, recipename, feedbackBy } = req.body
        const { db, body } = req
        const response = await createFeedbackRequest({ db, createdBy, recipename, feedbackBy, ...body })
...
const createFeedbackViewer = async (req, res, next) => {
    ...
    try {
        ...
        const { db, body } = req
        const response = await createFeedbackViewerRequest({ db, createdBy, recipename, feedbackBy, ...body })
        res.status(201).json({ message: "Feedback request created", data: response })
    } catch (error) {
        next(error)
    }
